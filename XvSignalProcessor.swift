...
open class XvSignalProcessorPacket {
    
    public init(raw:[Double], averagedValues:[Double], deviationValues:[Double], peaks:[Int]) {
        self.raw = raw
...
public class XvSignalProcessor {
    ...
    public func process(value:Double) -> XvSignalProcessorPacket? {
    ...
    //this adds to buffer, then buffer is passed into the peak detection method
    public func process(packet:[Double]) -> XvSignalProcessorPacket? {
    ...
    fileprivate func _process(rawSamples:[Double]) -> XvSignalProcessorPacket? {
        ...
        return XvSignalProcessorPacket(
            raw: rawSamples,
    ...
    fileprivate func _initArrays(){
        ...
        if (_averagingLag > _bins || _deviationLag > _bins) {
            print("XvSignalProcessor: Error: Lags (", _averagingLag, _deviationLag, ") can't be more than the bin length", _bins)
            fatalError()
        }
    }
