...
public class XvPeakDetector {
    ...
    public func process(yDataSet:[Double]) -> XvPeakDetectionPacket? {
        
        //MARK: Error checking
        //data set needs to match bin length
        if (yDataSet.count != _bins){
            print("XvPeakDetector: Error: Data set length", yDataSet.count, "doesn't match bin length", _bins)
            return nil
    ...
    public func detectPeak(from rawSamples:[Double]) -> XvPeakDetectionPacket? {
        ...
        return XvPeakDetectionPacket(peaks: _peaks, averagedValues: _avgFilter, deviationValues: _devFilter)
    ...
    fileprivate func _trimBuffer() -> Bool{
        ...
        if (_buffer.count < _bins) {
            print("XvPeakDetector: Building buffer", _buffer.count, "/", _bins)
            return false
        }
